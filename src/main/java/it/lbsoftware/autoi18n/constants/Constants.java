package it.lbsoftware.autoi18n.constants;

public final class Constants {

  public static final String AUTOI18N_NAME = "autoi18n";
  public static final String OPTION_SHORT_ENTRY = "-e";
  public static final String OPTION_LONG_ENTRY = "--entry";
  public static final String OPTION_SHORT_INPUT_LANGUAGE = "-i";
  public static final String OPTION_LONG_INPUT_LANGUAGE = "--input-language";
  public static final String OPTION_SHORT_TRANSLATION_ENGINE = "-t";
  public static final String OPTION_LONG_TRANSLATION_ENGINE = "--translation-engine";
  public static final String OPTION_SHORT_OUTPUT_LANGUAGES = "-o";
  public static final String OPTION_LONG_OUTPUT_LANGUAGES = "--output-languages";

  private Constants() {
    throw new UnsupportedOperationException("This class cannot be instantiated");
  }
}
